...
#SBATCH --output=gwas_miami.log   # Standard output and error log
...
if (filetype1 == "gzfile") {
  ...
  rawdata1 = read.table(paste0(opt$top), header = TRUE, blank.lines.skip = TRUE)
  # cat("\n* Select the appropiate columns...")
#   rawdata1 <- rawdata1 %>%
#   select( "VARIANTID", "CHR", "POS", "P_FIXED")
...
# change column names to required input
colnames(rawdata1)[1] <- c('SNP')
colnames(rawdata1)[2] <- c('CHR')
colnames(rawdata1)[3] <- c('POS')
colnames(rawdata1)[4] <- c('pvalue')
...
if (filetype2 == "gzfile") {
  ...
  rawdata2 = read.table(paste0(opt$bottom), header = TRUE, blank.lines.skip = TRUE)
    cat("\n* Select the appropiate columns...")
  # rawdata2 <- rawdata2 %>%
#   select( "VARIANTID", "CHR", "POS", "P_FIXED")
...
colnames(rawdata2)[1] <- c('SNP')
colnames(rawdata2)[2] <- c('CHR')
colnames(rawdata2)[3] <- c('POS')
colnames(rawdata2)[4] <- c('pvalue')
...
if (opt$imageformat == "PDF") {
	...
	gmirror(top=datatop, bottom=databottom, tline=0.05/nrow(datatop), bline=0.05/nrow(databottom), chrcolor1="palevioletred1", chrcolor2="royalblue1",
          toptitle = opt$toptitle, bottomtitle = opt$bottomtitle, 
          highlight_p = c(0.01/nrow(datatop),0.01/nrow(databottom)), highlighter="mediumorchid")
  }
