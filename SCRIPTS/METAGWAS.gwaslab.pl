...
#
#	 VariantID RSID CHR BP Beta SE P EffectAllele OtherAllele EAF Info
...
while(my $c = <REFFREQ>){
  ...
  # print STDOUT " *** DEBUG *** The $variant has allele frequency = $reference_a1_freq{$variant} and allele A1 = $a1; allele A2 = $a2.\n";
    ...
    # 1000G based
    elsif ( $reference eq "19" || $reference eq "1Gp1" || $reference eq "1Gp3" || $reference eq "GoNL4" || $reference eq "GoNL5" || $reference eq "1Gp3GONL5" ) {
...
print OUT "DIRECTIONS GENES_" . $gene_dist . "KB NEAREST_GENE NEAREST_GENE_ENSEMBLID NEAREST_GENE_STRAND VARIANT_FUNCTION CAVEAT RSID\n";
...
for (my $nvariant; $nvariant < $n_total_variants; $nvariant++) {
  ...
  for ( my $study = 0; $study < $nstudies; $study++ ) {
    ...
    if ( $fields[2] eq "X" ) { $chr[$study] = 23; }
    elsif ( $fields[2] eq "Y" ) { $chr[$study] = 24; }
    elsif ( $fields[2] eq "XY" ) { $chr[$study] = 25; }
    elsif ( $fields[2] eq "MT" ) { $chr[$study] = 26; }
    else { $chr[$study] = $fields[2]; }
    ...
    $se[$study] = $fields[5];
    $pval[$study] = $fields[6];
    $a1[$study] = allele_1234_to_ACGT( $fields[7] ); # coded/effect allele which is compared to the minor allele from HapMap2/1000G
    $a2[$study] = allele_1234_to_ACGT( $fields[8] ); # other/reference/major allele
    $af1[$study] = $fields[9];  
  ...
  for ( my $study=0; $study < $nstudies; $study++ ) {  ### START OF FOR-LOOP
    ...
    if ( $rsid[$study] eq "NA" || $beta[$study] eq "NA" || $se[$study] eq "NA" || $af1[$study] eq "NA" || $ratio[$study] eq "NA" || $a1[$study] eq "NA" || $a2[$study] eq "NA" || $se[$study] == 0 ) {
      $study_okay[$study] = 0;
      ...
      if ( $study_okay[$study] == 1 ) {
        ...
        if ( $sign == -1 ) {
          ...
          if ( $verbose ) {
            ...
            print OUT sprintf(" %s %s %s Y %.4f %.4f %.4f %.2e %.4f %.1f", $a1[$study], $a2[$study], $alleles_flipped, 1-$af1[$study], $rsid[$study], $signed_beta[$study], $se[$study], $pval[$study], $ratio[$study], $sample_size_eff[$study]);
          }
        } else {
          $af_weighted += ( $af1[$study] * $af_weight ); 
          if ( $verbose ) {
            print OUT sprintf(" %s %s %s N %.4f %.4f %.4f %.2e %.4f %.1f", $a1[$study], $a2[$study], $alleles_flipped, $af1[$study], $rsid[$study], $signed_beta[$study], $se[$study], $pval[$study], $ratio[$study], $sample_size_eff[$study]);
          }
        }
      }
      else {
...
#          print "gene = $gene[$i]   $dist_left  $dist_right  nearest gene = $nearest_gene   nearest_distance = $nearest_distance   gene_length_temp = $gene_length_temp\n"; 
    ...
    # Append RSID at the end
    my $rsid_out = "NA";
    for (my $study = 0; $study < $nstudies; $study++) {
        if (defined $rsid[$study] && $rsid[$study] ne "NA") {
            $rsid_out = $rsid[$study];
            last;
         }
     }
