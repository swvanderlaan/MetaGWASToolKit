...
if [ "$data_analysed" == "PHENOTYPE" ]; then
    ...
    bash ${LDSC}/ldsc.sumstats.sh -d ${DATA} -j "${PROJECTNAME}" -h ${SNPLIST} -p ${PHENOTYPE} -i ${DATA}/${PROJECTNAME}/${PHENOTYPE}/input/${PHENOTYPE}.cojo.gz -r ${r} -n ${n} -a ${a} -b ${b} -v ${v} -f ${f} -c ${CHUNKSIZE}
