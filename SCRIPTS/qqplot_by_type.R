...
### - add in more options for variant-types, currently only SNV and INDEL; 
###   one might think in terms missense/nonsense/etc., or eQTL/mQTL/pQTL, or other annotation

cat("++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    QQ by TYPE PLOTTER v1
    \n
    * Version: qq_by_type_plotter_v1
...
if(!is.na(opt$projectdir) & !is.na(opt$resultfile) & !is.na(opt$outputdir) & !is.na(opt$stattype) & !is.na(opt$imageformat)) {
  ...
  #data[,3] <- sapply(data[,1], function(x) ifelse(x<5.0e-8,5.0e-8,x))
  
  maxY <- round(max(-log10(data[,1]))) 	
  
  ### Stratify by TYPE
  type_snv=subset(data, ( data[,2] == "SNV" ))
  type_indel=subset(data, ( data[,2] == "INDEL" ))
  
  z=qnorm(data[,1]/2)
  z_snv=qnorm(type_snv[,1]/2)
  z_indel=qnorm(type_indel[,1]/2)
  
  
  ### CALCULATES LAMBDA AND # SNPs ###
  cat("Calculating lambda and number of variants from data.\n")
  ...
  plot(c(0,maxY),c(0,maxY),col = "#E55738",lwd = 1,type = "l",
       ...
       xaxs = "i",yaxs = "i",bty = "l",
       main=c(substitute(paste("QQ-plot stratified by variant type - ",lambda," = ", lam),list(lam = lambda)),expression()))
...
#--------------------------------------------------------------------------
### CLOSING MESSAGE
cat(paste("All done plotting a QQ-plot stratified by variant type of",study,".\n"))
cat(paste("\nToday's: ",Today, "\n"))
cat("++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++\n")
