...
cat("++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    QQ by CAF PLOTTER v1.1
    \n
    * Version: qq_by_caf_plotter_v1.1
    ...
    * Description: QQ-Plotter for GWAS (meta-analysis) results stratified 
      ...
      - second with coded/minor/effect/risk allele frequency (CAF)
      NO HEADER.
...
cat("\n++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++\n")
cat("Wow. We are finally starting \"QQ-Plotter - stratification by coded allele frequency\". ")
#--------------------------------------------------------------------------
...
if(!is.na(opt$projectdir) & !is.na(opt$resultfile) & !is.na(opt$outputdir) & !is.na(opt$stattype) & !is.na(opt$imageformat)) {
  ...
  cat(paste("We are going to \nmake the QQ-plot stratified by CAF of your (meta-)GWAS results. \nData are taken from.....: '",study,"'\nand will be outputed in.....: '", opt$outputdir, "'.\n",sep=''))
     ...
     ### Stratify by MAF
     pvals_lo1=subset(S, ( S[,2] > 0.20 & S[,2] < 0.80 ))
     pvals_lo2=subset(S, ( S[,2] < 0.20 & S[,2] > 0.05 ) | ( S[,2] > 0.80 & S[,2] < 0.95 ))
     pvals_lo3=subset(S, ( S[,2] < 0.05 & S[,2] > 0.01 ) | ( S[,2] > 0.95 & S[,2] < 0.99 ))
     pvals_lo4=subset(S, S[,2] < 0.01 | S[,2] > 0.99)
     ...
     cat("Calculating lambda from data.\n")
     lambda = round(median(z^2)/qchisq(0.5, df = 1),3)
     lambda
     n_snps = formatC(length(z), format="d", big.mark=',')
     n_snps
     
     study <- sub("\\.[[:alnum:]]+$", "", file_path_sans_ext(basename(opt$resultfile)))
     ...
     plot(c(0,maxY),c(0,maxY),col = "#E55738",lwd = 1,type = "l",
          ...
          xaxs = "i",yaxs = "i",bty = "l",
          main=c(substitute(paste("QQ-plot stratified by allele frequency - ",lambda," = ", lam),list(lam = lambda)),expression()))
     
     ### PLOTS DATA ###
     cat("\n- Plotting data.") 
     plotQQ(z,"black",0.5);
     plotQQ(z_lo1,"#9FC228",0.5);
     plotQQ(z_lo2,"#DB003F",0.5);
     plotQQ(z_lo3,"#1290D9",0.5);
     plotQQ(z_lo4,"#595A5C",0.5);
...
#--------------------------------------------------------------------------
### CLOSING MESSAGE
cat(paste("All done plotting a QQ-plot stratified by coded allele frequency of",study,".\n"))
cat(paste("\nToday's: ",Today, "\n"))
cat("++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++\n")
...
###save.image(paste0(opt$outputdir,"/",Today,"_",study,"_QQPLOT_BY_CAF.RData"))
