...
plot_snp_effect = function(expo_str, outcome_str, bxy, bzx, bzx_se, bzy, bzy_se, effect_col=colors()[75]) {
    ...
    plot(bzx, bzy, pch=20, cex=0.8, bty="n", cex.axis=1.1, cex.lab=1.2,
         ...
         ylab="", # Remove ylab from here
         main="")
    
    # Add the y-axis title with adjusted margin
    title(ylab=substitute(paste(trait, " (", italic(b[zy]), ")", sep=""), list(trait=outcome_str)),
          mgp=c(2, 1, 0))  # Adjust the first value as needed
