...
#	 VariantID	CHR	BP	Strand	EffectAlle	OtherAllele	EAF	MAF	MAC	HWE_P	Info	Beta	SE	P	N	N_cases	N_controls	Imputed	Reference

# 		1.  VariantID -- will be based on the reference, or contain markers from column "Marker" if not found via gwas2ref.harmonizer.py
# 		2.  CHR
# 		3.  BP
# 		4.  Strand
# 		5.  EffectAllele
...
# 		17. N_controls
# 		18. Imputed
#		19. Reference -- found in reference? via gwas2ref.harmonizer.py
...
#    CAF refers to the allele frequency of the coded allele (CA), 
#    RATIO is the ratio of the observed variance of the dosage to the expected (binomial) variance,
#    MAF is the minor allele frequency for the given SNP,
#    MAC is the minor allele count for the given SNP, and
#    N is the total sample size for the given SNP.
#
#    Columns 2-13 are repeated (on the same line) for every additional GWAS that is part of the
#    meta-analysis.
#
...
#
#	 Note: 	Here a SNP can be a single-nucleotide polymorphism or another type of variant such as
...
#
# 3) [--snps]	A PLINK BIM file -- for SNP positions 
#
...
#if ( ! $paramsFile || ! $snpFile || ! $dbsnpFile || ! $reffreqFile || ! $genesFile || ! $freqFilter || ! $freqWarning ) {
if ( ! $paramsFile || ! $snpFile || ! $dbsnpFile || ! $reffreqFile || ! $genesFile ) {
...
while(my $c = <HM>){
  ...
  if ( ( ! defined( $skip_list{$snp} ) ) && defined( $snplist{$snp} ) && ( ( ! $extractFile ) || defined( $extract{$snp} ) ) ) {
    ...
    $reference_a1_freq{$snp} = $fields[4];
	...
	$dbsnp_a2{$snp} = $tmp2;
      ...
      print STDERR "For $snp, cannot determine reference frequency for alleles $a1 and $a2 and annotated alleles $dbsnp_a1{$snp} $dbsnp_a2{$snp} -- skipping it\n";
      $skip_list{$snp} = 1;
    }
  }
}
...
print OUT "SNP CHR POS A1 A2 REFERENCE_A1_FREQ";
...
####################################################################################################
###
### LOOP OVER ALL SNPs FROM THE LIST -- and do some work
###
####################################################################################################
####################################################################################################
...
#  my $ref2 = "0";
  ...
  ### check if this SNP has a reference frequency 
  if ( ! defined( $reference_a1_freq{$snp} ) ) {
    $not_on_reference++;
    $reference_present{$snp} = 0;
    $caveat{$snp} .= "reference_present";
  }
  else {
      $reference_present{$snp} = 1;
  }
    ...
    if ( $beta[$study] eq "NA" || $se[$study] eq "NA" || $af1[$study] eq "NA" || $ratio[$study] eq "NA" || $a1[$study] eq "NA" || $a2[$study] eq "NA" || $se[$study] == 0 ) {
      ...
      if ( ( $a1[$study] eq $ref1 && $a2[$study] eq $ref2 ) || ( $a1[$study] eq $ref2 && $a2[$study] eq $ref1 ) ) { 
		...
		if ( ( $a1[$study] eq "A" && $a2[$study] eq "T" ) || ( $a1[$study] eq "T" && $a2[$study] eq "A" ) || ( $a1[$study] eq "C" && $a2[$study] eq "G" ) || ( $a1[$study] eq "G" && $a2[$study] eq "C" ) ) {
			if ( $reference_present{$snp} = 1 ) {
				if ( $a1[$study] eq $ref1 && ( $af1[$study] > ( $reference_a1_freq{$snp} - 0.15 ) ) && ( $af1[$study] < ( $reference_a1_freq{$snp} + 0.15 ) ) ) {
			}
			elsif ( $a2[$study] eq $ref1 && ( $af1[$study] > ( 1 - $reference_a1_freq{$snp} - 0.15 ) ) && ( $af1[$study] < ( 1 - $reference_a1_freq{$snp} + 0.15 ) ) ) {
			}
			elsif ( $a1[$study] eq allele_flip( $ref1 ) && ( $af1[$study] > ( $reference_a1_freq{$snp} - 0.15 ) ) && ( $af1[$study] < ( $reference_a1_freq{$snp} + 0.15 ) ) ) {
				$flip_alleles[$study] = 1;
			}
			elsif ( $a2[$study] eq allele_flip( $ref1 ) && ( $af1[$study] > ( 1 - $reference_a1_freq{$snp} - 0.15 ) ) && ( $af1[$study] < ( 1 - $reference_a1_freq{$snp} + 0.15 ) ) ) {
				$flip_alleles[$study] = 1;
			}
		else {
		    print STDERR "In $study_name[$study], $snp alleles ($a1[$study]/$a2[$study]) match the reference, but has allele frequencies inconsistent with the reference frequencies -- skipping this A/T or C/G SNP for this study\n";
		    $study_okay[$study] = 0;
		}
	    }
	}
       ...
       ### frequency-based test for non-A/T and non-C/G SNPs [THE CUTOFF FOR ALLELE FREQUENCY MATCHING SHOULD BE OPTIONAL]
		...
		if ( $a1[$study] eq $ref1 && ( $af1[$study] > ( $reference_a1_freq{$snp} - 0.15 ) ) && ( $af1[$study] < ( $reference_a1_freq{$snp} + 0.15 ) ) ) {
		    ...
		    print STDERR "In $study_name[$study], $snp alleles ($a1[$study]/$a2[$study]) match the reference, but has allele frequencies inconsistent with the reference frequencies -- skipping this non-A/T or non-C/G SNP for this study\n";
		    $study_okay[$study] = 0;
		}
	    }
      ...
      if ( ( ( $a1[$study] eq "A" && $a2[$study] eq "T" ) || ( $a1[$study] eq "T" && $a2[$study] eq "A" ) || ( $a1[$study] eq "C" && $a2[$study] eq "G" ) || ( $a1[$study] eq "G" && $a2[$study] eq "C" ) ) && ( ( $af1[$study] > 0.45 && $af1[$study] < 0.55 ) || ( $reference_a1_freq{$snp} > 0.45 && $reference_a1_freq{$snp} < 0.55 ) ) ) {
	  $caveat{$snp} .= "A/T_or_C/G_SNP_with_0.45<CAF<0.55";
      }
      ...
      ### frequency-based test for non-A/T and non-C/G SNPs [THE CUTOFF FOR ALLELE FREQUENCY MATCHING SHOULD BE OPTIONAL]
        if ( $reference_present{$snp} =1 ) {
	    if ( $a1[$study] eq allele_flip( $ref1 ) && ( $af1[$study] > ( $reference_a1_freq{$snp} - 0.15 ) ) && ( $af1[$study] < ( $reference_a1_freq{$snp} + 0.15 ) ) ) {
	    }
	    elsif ( $a2[$study] eq allele_flip( $ref1 ) && ( $af1[$study] > ( 1 - $reference_a1_freq{$snp} - 0.15 ) ) && ( $af1[$study] < ( 1 - $reference_a1_freq{$snp} + 0.15 ) ) ) {
	    }
	    else {
		print STDERR "In $study_name[$study], $snp alleles ($a1[$study]/$a2[$study]) do not match the reference, and has allele frequencies inconsistent with the reference frequencies -- skipping this non-A/T and non-C/G SNP for this study\n";
		$study_okay[$study] = 0;
	    }
        }
    }
  ...
  if ( $n_okay_studies > 0 && $n_eff > 0 ) {  
    ...
    ###
    ### we can now print out SNP information (based on reference, if available)
    ###
    print OUT "$snp $refchr $refpos $ref1 $ref2 ";
    if ( defined( $reference_a1_freq{$snp} ) ) { print OUT "$reference_a1_freq{$snp}"; } else { print OUT "NA"; }
