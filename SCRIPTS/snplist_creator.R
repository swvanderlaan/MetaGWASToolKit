...
male_summary <- read.table(paste0(MALE_loc, "meta.results.CHARGE_cIMT.1Gp3.EUR.summary.txt.gz"), header = TRUE, sep = "")


snplist <- male_summary %>% select("VARIANTID", "POS")

colnames(snplist)[1] <- c("IDS")

fwrite(snplist,
       file = paste0(MALE_loc, "meta.snplist.txt"),
