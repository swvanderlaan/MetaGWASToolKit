...
cat("++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    QQ by INFO-score PLOTTER v1.1
    \n
    * Version: qq_by_info_plotter_v1.1
...
# usage: ./qqplot_by_info.R -p projectdir -r resultfile -o outputdir -s stattype -f imageformat [OPTIONAL: -v verbose (DEFAULT) -q quiet]
#        ./qqplot_by_info.R --projectdir projectdir --resultfile resultfile --outputdir outputdir --stattype stattype --imageformat imageformat [OPTIONAL: --verbose verbose (DEFAULT) -quiet quiet]
...
if(!is.na(opt$projectdir) & !is.na(opt$resultfile) & !is.na(opt$outputdir) & !is.na(opt$stattype) & !is.na(opt$imageformat)) {
  ...
  cat(paste("We are going to \nmake the QQ-plot stratified by INFO-score of your (meta-)GWAS results. \nData are taken from.....: '",study,"'\nand will be outputed in.....: '", opt$outputdir, "'.\n",sep=''))
     ...
     plotQQ <- function(z,color,cex){
          ...
          # Samples 4,000 points from p > 1e-3
          cat("Sampling 4,000 points from p < 1e-3 (0.001).\n")
          n=4001
     ...
     # loading
     rawS <- read.table(opt$resultfile, header = FALSE)
     # removing NA's
     S <- na.omit(rawS)
     ...
     #Working on cutting of the P-value/statistic to maximum
     #S[,3]<- sapply(S[,1], function(x) ifelse(x<1.1e-7,1.1e-7,x))
     
     maxY <- round(max(-log10(S[,1]))) 	
     
     ### Stratify by INFO-metric/imputation quality (r^2)
     impqc_lo1=subset(S, ( S[,2] >= 0.0 & S[,2] <= 0.20 ))
     impqc_lo2=subset(S, ( S[,2] > 0.20 & S[,2] <= 0.40 ))
     impqc_lo3=subset(S, ( S[,2] > 0.40 & S[,2] <= 0.60 ))
     impqc_lo4=subset(S, ( S[,2] > 0.60 & S[,2] <= 0.80 ))
     impqc_lo5=subset(S, ( S[,2] > 0.80 & S[,2] <= 1.00 ))
     impqc_lo6=subset(S, ( S[,2] > 1.00 ))
     
     z=qnorm(S[,1]/2)
     z_lo1=qnorm(impqc_lo1[,1]/2)
     z_lo2=qnorm(impqc_lo2[,1]/2)
     z_lo3=qnorm(impqc_lo3[,1]/2)
     z_lo4=qnorm(impqc_lo4[,1]/2)
     z_lo5=qnorm(impqc_lo5[,1]/2)
     z_lo6=qnorm(impqc_lo6[,1]/2)
     
     ### CALCULATES LAMBDA AND # SNPs ###
     cat("\nCalculating lambda from data.")
     lambdavalue = round(median(z^2)/qchisq(0.5, df = 1),3)
     cat(paste0("",round(lambdavalue, digits = 4)))
     n_snps = formatC(length(z), format="d", big.mark=',')
     cat(paste0("",format(n_snps, big.mark = ",")))
...
#--------------------------------------------------------------------------
### CLOSING MESSAGE
cat(paste("All done plotting a QQ-plot stratified by INFO-score of",study,".\n"))
cat(paste("\nToday's: ",Today, "\n"))
cat("++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++\n")
...
###save.image(paste0(opt$outputdir,"/",Today,"_",study,"_QQPLOT_BY_INFO.RData"))
